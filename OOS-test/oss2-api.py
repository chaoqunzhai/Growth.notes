...
[docs]    def list_buckets(self, prefix='', marker='', max_keys=100):
        """根据前缀罗列用户的Bucket。
...
[docs]    def sign_url(self, method, key, expires, headers=None, params=None):
        """生成签名URL。
...
[docs]    def list_objects(self, prefix='', delimiter='', marker='', max_keys=100):
        """根据前缀罗列Bucket里的文件。
...
[docs]    def put_object(self, key, data,
                   headers=None,
...
[docs]    def put_object_from_file(self, key, filename,
                             headers=None,
                             progress_callback=None):
        """上传一个本地文件到OSS的普通文件。
...
[docs]    def append_object(self, key, position, data,
                      headers=None,
                      progress_callback=None):
        """追加上传一个文件。
...
[docs]    def get_object_to_file(self, key, filename,
                           byte_range=None,
                           headers=None,
                           progress_callback=None):
        """下载一个文件到本地文件。
...
[docs]    def head_object(self, key, headers=None):
        """获取文件元信息。
...
[docs]    def object_exists(self, key):
...
[docs]    def copy_object(self, source_bucket_name, source_key, target_key, headers=None):
        """拷贝一个文件到当前Bucket。
...
[docs]    def update_object_meta(self, key, headers):
...
[docs]    def delete_object(self, key):
        """删除一个文件。
...
[docs]    def put_object_acl(self, key, permission):
        """设置文件的ACL。
...
[docs]    def get_object_acl(self, key):
        """获取文件的ACL。
...
[docs]    def batch_delete_objects(self, key_list):
        """批量删除文件。待删除文件列表不能为空。
...
[docs]    def init_multipart_upload(self, key, headers=None):
        """初始化分片上传。
...
[docs]    def upload_part(self, key, upload_id, part_number, data, progress_callback=None):
        """上传一个分片。
...
[docs]    def complete_multipart_upload(self, key, upload_id, parts, headers=None):
        """完成分片上传，创建文件。
...
[docs]    def abort_multipart_upload(self, key, upload_id):
        """取消分片上传。
...
[docs]    def list_multipart_uploads(self,
                               prefix='',
                               delimiter='',
                               key_marker='',
                               upload_id_marker='',
...
[docs]    def upload_part_copy(self, source_bucket_name, source_key, byte_range,
...
[docs]    def list_parts(self, key, upload_id,
...
[docs]    def create_bucket(self, permission=None):
        """创建新的Bucket。
...
[docs]    def put_bucket_acl(self, permission):
        """设置Bucket的ACL。
...
[docs]    def put_bucket_cors(self, input):
        """设置Bucket的CORS。
...
[docs]    def delete_bucket_cors(self):
        """删除Bucket的CORS配置。"""
        resp = self.__do_bucket('DELETE', params={Bucket.CORS: ''})
        return RequestResult(resp)


[docs]    def put_bucket_lifecycle(self, input):
        """设置生命周期管理的配置。
...
[docs]    def get_bucket_lifecycle(self):
        """获取生命周期管理配置。
...
[docs]    def put_bucket_logging(self, input):
        """设置Bucket的访问日志功能。
...
[docs]    def get_bucket_logging(self):
        """获取Bucket的访问日志功能配置。
...
[docs]    def delete_bucket_logging(self):
        """关闭Bucket的访问日志功能。"""
        resp = self.__do_bucket('DELETE', params={Bucket.LOGGING: ''})
        return RequestResult(resp)


[docs]    def put_bucket_referer(self, input):
        """为Bucket设置防盗链。
...
[docs]    def get_bucket_referer(self):
        """获取Bucket的防盗链配置。
...
[docs]    def put_bucket_website(self, input):
        """为Bucket配置静态网站托管功能。
...
[docs]    def get_bucket_website(self):
        """获取Bucket的静态网站托管配置。
...
[docs]    def delete_bucket_website(self):
        """关闭Bucket的静态网站托管功能。"""
